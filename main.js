...
` + content);
    ...
    this.pullAllSafe = async () => {
      if (this.haltAllOperations) {
        new import_obsidian2.Notice("Operations are halted, aborting Pull All.");
        return;
      }
      if (!this.connectedToInternet) {
        new import_obsidian2.Notice("ERROR: No connectivity, aborting Pull All.");
        return;
      }
      const previousCount = this.settings.filesList ? this.settings.filesList.length : 0;
      new import_obsidian2.Notice("Pull All: refreshing file list from Google Drive...");
      await this.refreshFilesListInDriveAndStoreInSettings();
      const freshList = this.settings.filesList || [];
      if (freshList.length === 0 || previousCount > 0 && freshList.length < previousCount * 0.5) {
        new import_obsidian2.Notice(`SAFETY ABORT: cloud file list looks wrong (${freshList.length} files, expected around ${previousCount}). Nothing was touched. Check your connection/token and try again.`, 1e4);
        await this.writeToErrorLogFile(new Error(`Pull All aborted: suspicious filesList length ${freshList.length} (previous ${previousCount})`));
        return;
      }
      new import_obsidian2.Notice(`Pull All: downloading ${freshList.length} files (overwrite/create only, nothing will be deleted)...`, 8e3);
      let updated = 0, created = 0, skipped = 0, failed = 0;
      for (const cloudFile of freshList) {
        const path = cloudFile.name;
        if (!path || ignoreFiles.includes(path) || this.isInBlacklist(path)) {
          skipped++;
          continue;
        }
        try {
          const [, arrayBuffer] = await getFile(this.settings.accessToken, cloudFile.id);
          ...
          failed++;
          await this.writeToErrorLogFile(err);
          console.log("Pull All: failed for", path, err);
        }
      }
      new import_obsidian2.Notice(`Pull All finished. Updated: ${updated}, Created: ${created}, Skipped: ${skipped}, Failed: ${failed}. No local files were deleted.`, 12e3);
      await this.writeToVerboseLogFile(`LOG: Pull All finished. Updated: ${updated}, Created: ${created}, Skipped: ${skipped}, Failed: ${failed}`);
    };
    this.pushAllSafe = async () => {
      if (this.haltAllOperations) {
        new import_obsidian2.Notice("Operations are halted, aborting Push All.");
        return;
      }
      if (!this.connectedToInternet) {
        new import_obsidian2.Notice("ERROR: No connectivity, aborting Push All.");
        return;
      }
      new import_obsidian2.Notice("Push All: refreshing file list from Google Drive...");
      await this.refreshFilesListInDriveAndStoreInSettings();
      const cloudList = this.settings.filesList || [];
      const cloudByPath = /* @__PURE__ */ new Map();
      cloudList.forEach((f) => cloudByPath.set(f.name, f));
      const localFiles = this.app.vault.getFiles().filter((f) => !ignoreFiles.includes(f.path) && !this.isInBlacklist(f.path));
      new import_obsidian2.Notice(`Push All: uploading ${localFiles.length} local files (overwrite/create only, nothing will be deleted on Drive)...`, 8e3);
      let updated = 0, created = 0, failed = 0;
      for (const file of localFiles) {
        try {
          const buffer = await this.app.vault.readBinary(file);
          const cloudMatch = cloudByPath.get(file.path);
          if (cloudMatch) {
            await modifyFile(this.settings.accessToken, cloudMatch.id, buffer);
            updated++;
          } else {
          ...
          failed++;
          await this.writeToErrorLogFile(err);
          console.log("Push All: failed for", file.path, err);
        }
      }
      await this.refreshFilesListInDriveAndStoreInSettings();
      new import_obsidian2.Notice(`Push All finished. Updated: ${updated}, Created: ${created}, Failed: ${failed}. Nothing was deleted on Google Drive.`, 12e3);
      await this.writeToVerboseLogFile(`LOG: Push All finished. Updated: ${updated}, Created: ${created}, Failed: ${failed}`);
    };
  ...
  async onload() {
    ...
    this.registerEvent(this.app.vault.on("modify", async (e) => {
      ...
      try {
        ...
        if (this.timers.has(e.path))
          clearTimeout(this.timers.get(e.path));
        this.timers.set(e.path, setTimeout(async () => {
          if (e instanceof import_obsidian2.TFile) {
    ...
    this.addCommand({
      id: "drive-pull-all-safe",
      name: "Pull All from Google Drive (safe, never deletes)",
      callback: async () => {
        new ConfirmBulkSync(
          this.app,
          "Pull All",
          "This will download every file from Google Drive and OVERWRITE the matching local file (or create it if missing). It will NEVER delete a local file. Make sure you've made a backup first, then confirm.",
          async () => {
            await this.pullAllSafe();
          }
        ).open();
      }
    });
    this.addCommand({
      id: "drive-push-all-safe",
      name: "Push All to Google Drive (safe, never deletes)",
      callback: async () => {
        new ConfirmBulkSync(
          this.app,
          "Push All",
          "This will upload every local file to Google Drive and OVERWRITE the matching cloud file (or create it if missing). It will NEVER delete a file on Google Drive. Make sure you've made a backup first, then confirm.",
          async () => {
            await this.pushAllSafe();
          }
        ).open();
      }
    });
...
var ConfirmBulkSync = class extends import_obsidian2.Modal {
  ...
  onOpen() {
    ...
    new import_obsidian2.Setting(contentEl).setName(this.message);
    let confirmed = false;
    new import_obsidian2.Setting(contentEl).addText((text) => {
      text.setPlaceholder('Type "confirm" to proceed');
      text.onChange((value) => {
    ...
    new import_obsidian2.Setting(contentEl).addButton((btn) => btn.setButtonText("Cancel").onClick(() => {
      this.close();
    })).addButton((btn) => btn.setButtonText(`Run ${this.title}`).setWarning().onClick(() => {
      if (!confirmed) {
        new import_obsidian2.Notice('You must type "confirm" to proceed.');
        return;
      }
      this.close();
      this.onConfirm();
